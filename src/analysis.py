...
)  # Alias to avoid confusion with our parser module if any
...
try:
    ...
    logging.error(
        "Could not import from src.calendar_actions or src.models. Ensure structure is correct."
    )
...
def project_recurring_events(
    ...
    logger.info(
        f"Projection window: {time_min} to {time_max}. Query: '{event_query or 'None'}'"
    )
    ...
    for event in master_events_response.items:
        ...
        if not event.start or not (event.start.dateTime or event.start.date):
            logger.warning(
                f"Skipping recurring event without start time: {event.summary} ({event.id})"
            )
            continue
        ...
        if event.start.dateTime:
            try:
                ...
                if event.end and event.end.dateTime:
                    ...
                    logger.warning(
                        f"Recurring event '{event.summary}' missing end.dateTime, assuming {event_duration} duration."
                    )
            except ValueError as e:
                logger.error(
                    f"Could not parse dateTime for event {event.summary} ({event.id}): {e}"
                )
                continue
            ...
            try:
                ...
                if event.end and event.end.date:
                    ...
                    event_duration = (
                        end_date - start_date
                    )  # This includes the start day but excludes the end day
                else:
                    event_duration = timedelta(days=1)  # Assume single all-day event
            except ValueError as e:
                logger.error(
                    f"Could not parse date for event {event.summary} ({event.id}): {e}"
                )
                continue

        if not dtstart_obj or event_duration is None:
            logger.error(
                f"Could not determine dtstart or duration for event {event.summary} ({event.id})"
            )
            continue
        ...
        if not rrule_str:
            logger.warning(
                f"Recurring event '{event.summary}' ({event.id}) has no RRULE string. Skipping."
            )
            continue
        ...
        try:
            ...
            main_rule = rrule.rrulestr(
                rrule_str, dtstart=dtstart_obj, forceset=True
            )  # forceset=True to handle COUNT/UNTIL easily
                ...
                if len(parts) == 2:
                    ...
                    for date_str in dates:
                        try:
                            ...
                            logger.warning(
                                f"Could not parse EXDATE value '{date_str}' for event {event.id}"
                            )
            ...
            occurrences = ruleset.between(
                time_min, time_max, inc=True
            )  # inc=True includes time_min

            logger.debug(
                f"Event '{event.summary}' ({event.id}): Found {len(occurrences)} occurrences via rrule."
            )
            ...
            logger.error(
                f"Failed to parse/process recurrence for event '{event.summary}' ({event.id}): {e}",
                exc_info=True,
            )
            continue  # Skip this event

    logger.info(
        f"Finished projection. Found {len(projected_occurrences)} total occurrences."
    )
...
def analyze_busyness(
    ...
    """
    busyness_by_date: Dict[date, Dict[str, Any]] = defaultdict(
        lambda: {"event_count": 0, "total_duration_minutes": 0.0}
    )
    ...
    if not events_response or not events_response.items:
        logger.info(
            "No events found in the specified time range for busyness analysis."
        )
        return dict(
            busyness_by_date
        )  # Return empty default dict converted to regular dict
    ...
    for event in events_response.items:
        ...
        if event.start:
            if event.start.dateTime:
                try:
                    ...
                    logger.warning(
                        f"Could not parse start dateTime: {event.start.dateTime}"
                    )
                    continue
            elif event.start.date:
                try:
        ...
        if not event_date:
            logger.warning(
                f"Event '{event.summary}' ({event.id}) missing valid start information. Skipping."
            )
            continue
        ...
        if start_dt and event.end and event.end.dateTime:
            try:
                ...
                busyness_by_date[event_date]["total_duration_minutes"] += max(
                    0, duration.total_seconds() / 60.0
                )
            except ValueError:
                logger.warning(
                    f"Could not parse end dateTime: {event.end.dateTime} for event {event.id}"
                )
            except TypeError:
                logger.warning(
                    f"Could not calculate duration for event {event.id} (start: {start_dt}, end: {end_dt})"
                )
