...
class EventDateTime(BaseModel):
    ...
    dateTime: Optional[datetime.datetime] = (
        None  # Renamed from 'date_time' to match API JSON
    )
...
class EventCreator(BaseModel):
    ...
    self: Optional[bool] = (
        None  # Whether the creator corresponds to the calendar on which this copy of the event appears.
    )
...
class EventOrganizer(BaseModel):
    ...
    display_name: Optional[str] = Field(None, alias="displayName")
    self: Optional[bool] = (
        None  # Whether the organizer corresponds to the calendar on which this copy of the event appears.
    )
...
class GoogleCalendarEvent(BaseModel):
    ...
    status: Optional[str] = Field(
        None, description="Status of the event ('confirmed', 'tentative', 'cancelled')."
    )
    html_link: Optional[str] = Field(
        None,
        alias="htmlLink",
        description="URL for the event in the Google Calendar UI.",
    )
    created: Optional[datetime.datetime] = Field(
        None, description="Creation time of the event (RFC3339 format)."
    )
    updated: Optional[datetime.datetime] = Field(
        None, description="Last modification time of the event (RFC3339 format)."
    )
    summary: Optional[str] = Field(None, description="Title of the event.")
    description: Optional[str] = Field(
        None, description="Description of the event. Optional."
    )
    location: Optional[str] = Field(
        None, description="Geographic location of the event. Optional."
    )
    color_id: Optional[str] = Field(
        None, alias="colorId", description="Color of the event. Optional."
    )
    creator: Optional[EventCreator] = Field(
        None, description="The creator of the event. Read-only."
    )
    organizer: Optional[EventOrganizer] = Field(
        None, description="The organizer of the event."
    )
    start: Optional[EventDateTime] = Field(
        None, description="The start time of the event."
    )
    ...
    recurrence: Optional[List[str]] = Field(
        None,
        description="List of RRULE, EXRULE, RDATE or EXDATE properties for recurring events.",
    )
    recurring_event_id: Optional[str] = Field(
        None,
        alias="recurringEventId",
        description="For an instance of a recurring event, this is the id of the recurring event itself.",
    )
    original_start_time: Optional[EventDateTime] = Field(
        None,
        alias="originalStartTime",
        description="For an instance of a recurring event, this is the original start time of the instance before modification.",
    )
    attendees: Optional[List[EventAttendee]] = Field(
        [], description="The attendees of the event."
    )
    attendees_omitted: Optional[bool] = Field(
        None, alias="attendeesOmitted", description="Whether attendees were omitted."
    )
    reminders: Optional[EventReminders] = Field(
        None, description="Information about the event's reminders."
    )
...
class EventCreateRequest(BaseModel):
    ...
    attendees: Optional[List[EmailStr]] = Field(
        None, description="List of attendee email addresses to invite."
    )
    recurrence: Optional[List[str]] = Field(
        None,
        description="List of RRULEs, EXRULEs, RDATEs or EXDATEs for recurring events.",
    )
    reminders: Optional[EventReminders] = Field(
        None, description="Notification settings for the event."
    )
...
class QuickAddEventRequest(BaseModel):
    ...
    text: str = Field(
        ...,
        description="The text describing the event to be parsed by Google Calendar.",
    )
...
class EventUpdateRequest(BaseModel):
    ...
    attendees: Optional[List[EventAttendee]] = (
        None  # Allow updating attendee details or list
    )
...
class AddAttendeeRequest(BaseModel):
    ...
    attendee_emails: List[EmailStr] = Field(
        ..., description="List of email addresses to add as attendees."
    )
...
class NotificationSettings(BaseModel):
    ...
    notifications: Optional[List[Dict[str, str]]] = (
        None  # List of {'type': 'eventCreation', 'method': 'email'} etc.
    )
...
class CalendarListEntry(BaseModel):
    ...
    defaultReminders: Optional[List[EventReminderOverride]] = (
        None  # Renamed from 'default_reminders'
    )
    notificationSettings: Optional[NotificationSettings] = (
        None  # Renamed from 'notification_settings'
    )
...
class CheckAttendeeStatusResponse(BaseModel):
    status_map: Dict[EmailStr, str] = Field(
        ...,
        description="Mapping of attendee email to their responseStatus ('accepted', 'declined', etc.)",
    )
...
class ScheduleMutualRequest(BaseModel):
    attendee_calendar_ids: List[str] = Field(
        ...,
        description="List of calendar IDs (usually emails) for attendees whose availability should be checked.",
    )
    ...
    working_hours_start_str: Optional[str] = Field(
        None,
        description="Optional start time for working hours constraint (HH:MM format)",
    )
    working_hours_end_str: Optional[str] = Field(
        None,
        description="Optional end time for working hours constraint (HH:MM format)",
    )
...
class AnalyzeBusynessResponse(BaseModel):
    ...
    busyness_by_date: Dict[str, DailyBusynessStats] = Field(
        ..., description="Mapping of date string (YYYY-MM-DD) to busyness stats"
    )
