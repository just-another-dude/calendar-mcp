...
class OAuthCallbackHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if code:
            ...
            self.wfile.write(
                b"<p>Authorization code received. You can close this window.</p></body></html>"
            )
        elif error:
            self.error = error[0]
            ...
            self.wfile.write(
                f"<p>Error: {self.error}. Please check console.</p></body></html>".encode()
            )
        else:
...
def get_credentials(user_id: str = None):
    ...
    if os.path.exists(user_token_file):
        try:
            ...
            logger.warning(
                f"Failed to load credentials from {user_token_file}: {e}. Will attempt re-authentication."
            )
        ...
        if creds and creds.expired and creds.refresh_token:
            ...
            if creds:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to save credentials to {user_token_file}: {e}"
                    )
            else:
                logger.error(
