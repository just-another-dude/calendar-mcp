...
async def test_service_account_fallback():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            print(
                f"📤 Sending MCP request with invalid OAuth token: {invalid_token[:25]}..."
            )
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if "error" in result:
                        ...
                        error_type = error_data.get("data", {}).get(
                            "error_type", "unknown"
                        )
                        ...
                        if error_type == "oauth_refresh_failed":
                            service_available = error_data.get("data", {}).get(
                            ...
                            if not service_available:
                                ...
                                print(
                                    "  - Service account not configured (expected for local testing)"
                                )
                                print(
                                    "  - This will work in Railway with service account configuration"
                                )
                            else:
                                print(
                            ...
                            oauth_error = error_data.get("data", {}).get(
                                "oauth_error", ""
                            )
                            sa_error = error_data.get("data", {}).get(
                            ...
                            print(
                                "  - Both failed (expected without service account configuration)"
                            )
